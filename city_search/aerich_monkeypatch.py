...
"""
Monkeypatch fix for aerich migration.

Fix for this bug of migrating many-to-many fields
https://github.com/tortoise/aerich/issues/150
"""
    ...
    def diff_models(
            ...
            if new_model_str not in old_models.keys():
                ...
                old_indexes = set(
                    map(
                        ...
                        cls._handle_indexes(
                            model, old_model_describe.get("indexes", [])
                        ),
                    )
                )
                new_indexes = set(
                    map(
                        lambda x: x if isinstance(x, Index) else tuple(x),
                        cls._handle_indexes(
                            model, new_model_describe.get("indexes", [])
                        ),
                    )
                )
                ...
                for index in new_indexes.difference(old_indexes):
                    cls._add_operator(
                        cls._add_index(model, index, False), upgrade, True
                    )
                # remove indexes
                for index in old_indexes.difference(new_indexes):
                    cls._add_operator(
                        cls._drop_index(model, index, False), upgrade, True
                    )
                old_data_fields = old_model_describe.get("data_fields")
                new_data_fields = new_model_describe.get("data_fields")

                old_data_fields_name = list(
                    map(lambda x: x.get("name"), old_data_fields)
                )
                new_data_fields_name = list(
                    map(lambda x: x.get("name"), new_data_fields)
                )
                ...
                for new_data_field_name in set(new_data_fields_name).difference(
                    ...
                    new_data_field = next(
                        filter(
                            lambda x: x.get("name") == new_data_field_name,
                            new_data_fields,
                        )
                    )
                    is_rename = False
                ...
                for old_data_field_name in set(old_data_fields_name).difference(
                    ...
                    cls._add_operator(
                        cls._remove_field(
                            ...
                            next(
                                filter(
                                    lambda x: x.get("name") == old_data_field_name,
                                    old_data_fields,
                                )
                            ).get("db_column"),
                        ),
                ...
                for new_fk_field_name in set(new_fk_fields_name).difference(
                    ...
                    fk_field = next(
                        filter(
                            lambda x: x.get("name") == new_fk_field_name, new_fk_fields
                        )
                    )
                    if fk_field.get("db_constraint"):
                        cls._add_operator(
                            cls._add_fk(
                                model,
                                fk_field,
                                new_models.get(fk_field.get("python_type")),
                            ),
                            upgrade,
                            fk_m2m_index=True,
                        )
                ...
                for old_fk_field_name in set(old_fk_fields_name).difference(
                    ...
                    old_fk_field = next(
                        filter(
                            lambda x: x.get("name") == old_fk_field_name, old_fk_fields
                        )
                    )
        ...
        for old_model in old_models:
            if old_model not in new_models.keys():
                cls._add_operator(
                    cls.drop_model(old_models.get(old_model).get("table")), upgrade
                )
